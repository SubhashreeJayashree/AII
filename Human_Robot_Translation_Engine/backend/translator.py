        ...
        if subj and subj in subjects_questions:
            ...
            if query_norm in pool_index:
                ...
                if matches:
                    ...
                    return {
                        'steps': ['Identify subject', f'Subject selected: {subj}', 'No exact match', 'Provide related Q/A'],
                        'speech': speech_output,
                        'output': {'quiz': enriched}
                    }
            speech_output = f"Q: {task}\nA: {answer}"
            return {
                'steps': ['Identify subject', f'Subject selected: {subj}', 'Find best match', 'Present answer'],
                'speech': speech_output,
                'output': {'quiz': [{'q': task, 'a': answer}] }
            }
            ...
            if query_norm in combined_index:
                ...
                if matches:
                    ...
                    return {
                        'steps': ['Identify subject (none)', 'Search across pools', 'No exact match', 'Provide related Q/A'],
                        'speech': speech_output,
                        'output': {'quiz': enriched_texts}
                    }
            speech_output = f"[{matched_subject}] Q: {task}\nA: {answer}"
            return {
                'steps': ['Identify subject (inferred)', f'Matched subject: {matched_subject}', 'Present answer'],
                'speech': speech_output,
                'output': {'quiz': [{'q': task, 'a': answer, 'subject': matched_subject}] }
            }
